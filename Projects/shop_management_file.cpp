...
// default=10,, & <20 quantity for each unique comp
...
static int productIndex = 1; // total unique prod present till for respective companys
static int companyIndex = 1; // total num of company present till
...
int removeAllComp();
int exitPrgm();
void updateDate_autoProd(short unsigned int &, short unsigned int &); // for random prod's date
// main function..
int main()
{
    fstream prod_file;
    prod_file.open("products_file.txt", ios::out | ios::in);
